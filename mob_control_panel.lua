-- =========================================================
-- MOB CONTROL PANEL
-- Master controls, maintenance lockout, fan clear-out timer,
...
local theme = {
    ...
    maintenanceOn=colors.orange,maintenanceOff=colors.gray,
...
local spawners = {}
local buttons = {
    masterOn={},masterOff={},fans={},maintenance={},refresh={},reboot={}
}
...
local refresh = {
    visible=false,running=false,started=0,finishedAt=0,
    expected={},responses={}
}
...
local function saveMaintenance()
    ...
    if f then
        f.write(maintenanceMode and "true" or "false")
        f.close()
    end
end
...
local function emptySpawner()
    return {
        ...
        lastUpdate=0,seenSinceBoot=false,x1=0,y1=0,x2=0,y2=0,
        titleY=0,borderPoints={}
    }
end
...
local function calculateLayout()
    ...
    buttons.reboot.x1=buttons.reboot.x2-bw+1
    buttons.refresh.x2=buttons.reboot.x1-gap-1
    buttons.refresh.x1=buttons.refresh.x2-bw+1
    buttons.maintenance.x2=buttons.refresh.x1-gap-1
    buttons.maintenance.x1=buttons.maintenance.x2-bw+1
    buttons.fans.x2=buttons.maintenance.x1-gap-1
    buttons.fans.x1=buttons.fans.x2-bw+1
    buttons.masterOff.x2=buttons.fans.x1-gap-1
    buttons.masterOff.x1=buttons.masterOff.x2-bw+1
    buttons.masterOn.x2=buttons.masterOff.x1-gap-1
    buttons.masterOn.x1=buttons.masterOn.x2-bw+1
...
local function drawMaintenanceButton()
    if maintenanceMode then
        drawButton(buttons.maintenance,"MAINT: ON",theme.maintenanceOn)
    else
        drawButton(buttons.maintenance,"MAINT",theme.maintenanceOff)
    end
end
...
local function drawHeader()
    ...
    local subtitle="DYNAMIC MOB MANAGEMENT"
    local subtitleColor=theme.subtext

    if maintenanceMode then
        subtitle="MAINTENANCE LOCKOUT - ALL OUTPUTS OFF"
        subtitleColor=colors.red
    elseif pinkSlimeAutomationActive then
        subtitle="PINK SLIME AUTO ACTIVE"
        subtitleColor=colors.red
    end

    writeAt(3,3,subtitle,subtitleColor,theme.header)
...
local function drawSpawner(s,index)
    ...
    elseif not s.online then
        border(
            s.x1,s.y1,s.x2,s.y2,
            math.floor(animationFrame/3)%2==0 and theme.offlineA or theme.offlineB
        )
    elseif not s.state then
        border(s.x1,s.y1,s.x2,s.y2,theme.off)
    else
        local pattern={
            theme.runningDark,theme.runningDark,theme.runningDark,
            theme.runningBright,theme.runningBright,theme.runningDark
        }
        for i,p in ipairs(s.borderPoints) do
            writeAt(
                p.x,p.y," ",colors.white,
                pattern[((i+animationFrame-2)%#pattern)+1]
            )
        end
    end
end
...
local function drawFooter()
    ...
    if pinkSlimeAutomationActive and not maintenanceMode then
        right=right.."PINK AUTO  "
    end
    right=right..(fansState and "FANS ON" or "FANS OFF")

    writeAt(
        math.max(2,width-#right),height,
        right,
...
local function drawRefreshPopup()
    ...
    local expected=refreshCount(refresh.expected)
    local responses=refreshCount(refresh.responses)
    ...
    for _,id in ipairs(ids) do
        ...
        if result then
            writeAt(
                x1+3,row,result.success and "+" or "X",
                result.success and theme.success or theme.failure,
                theme.popup
            )
            writeAt(x1+5,row,label,colors.white,theme.popup)
            writeAt(
                x2-9,row,result.success and "UPDATED" or "FAILED",
                result.success and theme.success or theme.failure,
                theme.popup
            )
            if not result.success and result.error and row+1<y2-1 then
                row=row+1
                writeAt(
                    x1+7,row,shorten(result.error,pw-12),
                    theme.failure,theme.popup
                )
            end
        else
    ...
    center(
        x1+2,x2-2,y2-1,
        refresh.running and "WAITING FOR NODES..." or "RESULTS CLOSE AUTOMATICALLY",
...
local function sendSpawnerState(s)
    ...
    -- Belt-and-suspenders safety: never send ON while locked out.
    ...
    rednet.send(
        s.computerID,
        {command="spawn",spawnerKey=s.spawnerKey,state=desired},
        controlProtocol
    )
end
...
local function masterOn()
    if maintenanceMode then
        -- Lockout means absolutely nothing can turn on.
...
local function masterOff()
    ...
    if maintenanceMode then
        ...
        -- Normal shutdown: fans clear the room for 30 seconds.
        fansState=true
        sendFansState()
...
local function applyMaintenanceLockout()
    ...
    -- Cancel any automatic mob-farm override. Do NOT restore its
    -- snapshot later; maintenance intentionally leaves everything OFF.
    ...
    -- Remote nodes also enforce the lock locally, so even a stray
    -- ON command cannot energize a spawner.
    rednet.broadcast(
...
local function setMaintenance(state)
    ...
    if maintenanceMode then
        ...
        -- Unlock only. Everything stays OFF until manually or
        -- automatically requested again.
        rednet.broadcast(
            {command="maintenance",state=false,timestamp=now()},
            controlProtocol
        )
        setAllSpawners(false)
        setFans(false)
...
local function setPinkAutomation(requested)
    ...
    if maintenanceMode then
        -- Ignore Pink Slime ON requests while locked out.
        pinkSlimeAutomationActive=false
        pinkSlimeSnapshot=nil
        applyMaintenanceLockout()
        return
...
local function processMachineStatus(m)
    if type(m)=="table"
        and m.messageType=="resource_machine_status"
        and m.itemID==pinkSlimeItemID then
...
local function requestDiscovery()
    ...
    -- Reassert maintenance to nodes that may have rebooted.
    if maintenanceMode then
        rednet.broadcast(
            {command="maintenance",state=true,timestamp=now()},
            controlProtocol
...
local function processSpawnerStatus(senderID,m)
    ...
    elseif pinkSlimeAutomationActive and not s.state then
        s.state=true
        sendSpawnerState(s)
    end
...
local function processRefreshStatus(senderID,m)
    ...
    refresh.responses[senderID]={
        success=m.success==true,error=m.error,role=m.role,
        configSource=m.configSource,timestamp=now()
    }
...
local function updateRefresh()
    ...
    elseif refresh.finishedAt>0
        and t-refresh.finishedAt>=refreshResultSeconds*1000 then
    ...
    if event=="monitor_touch" and a==monitorName then
        ...
        if not refresh.visible then
            ...
            elseif not maintenanceMode then
                for i,s in ipairs(spawners) do
                    if s.computerID~=0
                        and x>=s.x1 and x<=s.x2
                        and y>=s.y1 and y<=s.y2 then
