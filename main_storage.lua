...
local function rememberComputer(computerID,role)
    ...
    if not info then info={}; knownComputers[computerID]=info end
...
local function drawPixel(x,y,color)
    monitor.setCursorPos(x,y); monitor.setBackgroundColor(color); monitor.write(" ")
end
...
local function processStorageManifest(senderID,message)
    ...
    for itemID in pairs(source) do if not enabled[itemID] then source[itemID]=nil end end
    screenDirty=true
end
...
local function processInventoryUpdate(senderID,message)
    ...
    local source=ensureStorageSource(senderID)
    source[itemID]={
        itemID=itemID, displayName=message.displayName or itemID,
        amount=tonumber(message.amount) or 0,
        targetAmount=tonumber(message.targetAmount or message.target) or 0,
        found=message.found~=false, online=message.online~=false,
        error=message.error, lastUpdate=now()
    }
    screenDirty=true
end
...
local function processMachineStatus(senderID,message)
    ...
    machineStates[message.itemID]={running=message.state==true,computerID=senderID,
        machineKey=message.machineKey,side=message.side,lastUpdate=now()}
    screenDirty=true
end
...
local function processRefreshStatus(senderID,message)
    ...
    refresh.responses[senderID]={success=message.success==true,error=message.error,
...
local function calculatePercentage(card)
    local amount=tonumber(card.amount); local target=tonumber(card.targetAmount)
...
local function rebuildCards()
    ...
    for _,source in pairs(storageSources) do
        for itemID,data in pairs(source) do
            ...
            if age>sourceRemoveSeconds*1000 then source[itemID]=nil
            else
                local card=combined[itemID]
                if not card then
                    card={itemID=itemID,name=data.displayName or itemID,amount=0,targetAmount=0,
                        found=true,online=false,error=nil,machineRunning=false,x1=0,y1=0,x2=0,y2=0,borderPoints={}}
...
local function firstIndexOnPage() return (currentPage-1)*cardsPerPage+1 end
local function lastIndexOnPage() return math.min(#cardOrder,currentPage*cardsPerPage) end

local function updatePageRotation()
    local pages=pageCount()
    if pages<=1 then
        if currentPage~=1 then currentPage=1; screenDirty=true end
        pageChangedAt=now(); return
    end
    ...
    if time-pageChangedAt>=pageSeconds*1000 then
        ...
        pageChangedAt=time; screenDirty=true
    end
end
...
local function calculateLayout()
    ...
    for index=1,cardsPerPage do
        ...
        local x2=x1+cardWidth-1; local y2=y1+cardHeight-1
        layoutSlots[index]={x1=x1,y1=y1,x2=x2,y2=y2,borderPoints=getBorderPoints(x1,y1,x2,y2)}
    end

    local rebootWidth=14; local refreshWidth=24
    rebootButton.x2=width-2; rebootButton.x1=rebootButton.x2-rebootWidth+1
    rebootButton.y1=2; rebootButton.y2=4
    refreshButton.x2=rebootButton.x1-2; refreshButton.x1=refreshButton.x2-refreshWidth+1
...
local function drawCenteredBarText(x1,x2,y,text,filledUntil,filledColor)
    local width=x2-x1+1; text=shortenText(text,width)
    local textX=x1+math.floor((width-#text)/2)
    for index=1,#text do
        local x=textX+index-1; local bg=theme.progressBackground
        if filledUntil and x<=filledUntil then bg=filledColor end
        writeAt(x,y,text:sub(index,index),theme.amountText,bg)
    end
end

local function drawProgressBar(card)
    local y=card.y1+3; local x1=card.x1+2; local x2=card.x2-2; local width=x2-x1+1
    fill(x1,y,x2,y,theme.progressBackground)
    if hasError(card) then drawCenteredBarText(x1,x2,y,getErrorText(card),nil,nil); return end
    local p=calculatePercentage(card); local filledWidth=math.floor(width*p/100)
    if p>0 and filledWidth<1 then filledWidth=1 end
    if filledWidth>width then filledWidth=width end
    local progressColor=getProgressColor(card); local filledUntil=nil
    if filledWidth>0 then filledUntil=x1+filledWidth-1; fill(x1,y,filledUntil,y,progressColor) end
    drawCenteredBarText(x1,x2,y,commaNumber(card.amount).." / "..commaNumber(card.targetAmount),filledUntil,progressColor)
end

local function drawMachineChase(card)
    if not card.machineRunning then return end
    local points=card.borderPoints; if not points or #points==0 then return end
    local startIndex=(chaseFrame%#points)+1
    for offset=0,chaseLength-1 do
        local index=(startIndex+offset-1)%#points+1; local point=points[index]
        drawPixel(point.x,point.y,theme.machineChase)
    end
end

local function drawCardBorder(card)
    drawBorder(card.x1,card.y1,card.x2,card.y2,getBorderColor(card)); drawMachineChase(card)
end
...
local function drawCard(card)
    ...
    centerInside(card.x1+2,card.x2-2,card.y1+2,string.upper(card.name or "STORAGE"),theme.title,theme.card)
...
local function countErrors()
    local count=0
    for _,itemID in ipairs(cardOrder) do if cards[itemID] and hasError(cards[itemID]) then count=count+1 end end
    return count
end

local function countRunning()
    local count=0
    for _,itemID in ipairs(cardOrder) do if cards[itemID] and cards[itemID].machineRunning then count=count+1 end end
    return count
end
...
local function drawFooter()
    ...
    local pages=pageCount(); if pages>1 then text=text.."  PAGE "..currentPage.."/"..pages end
...
local function getSuccessCount() local c=0 for _,r in pairs(refresh.responses) do if r.success then c=c+1 end end return c end
local function getFailureCount() local c=0 for _,r in pairs(refresh.responses) do if not r.success then c=c+1 end end return c end
...
local function resetRefresh()
    ...
    for computerID,info in pairs(knownComputers) do
        if time-(info.lastSeen or 0)<=sourceOfflineSeconds*1000 then refresh.expected[computerID]=true end
    end
end

local function drawRefreshPopup()
    if not refresh.visible then return end
    local width,height=monitor.getSize(); local popupWidth=math.min(60,width-8); local popupHeight=math.min(18,height-10)
    local x1=math.floor((width-popupWidth)/2)+1; local y1=math.floor((height-popupHeight)/2)+1
    local x2=x1+popupWidth-1; local y2=y1+popupHeight-1
    drawBorder(x1,y1,x2,y2,theme.popupBorder); fill(x1+1,y1+1,x2-1,y2-1,theme.popupBackground)
    ...
    local summary=refresh.running and ("UPDATING... "..tableCount(refresh.responses).." / "..tableCount(refresh.expected))
        or (getSuccessCount().." UPDATED / "..getFailureCount().." FAILED")
    ...
    for _,id in ipairs(ids) do
        ...
        local result=refresh.responses[id]; local info=knownComputers[id]
        local role=result and result.role or (info and info.role) or "NODE"; local label=tostring(role).." "..tostring(id)
        if result then
    ...
    centerInside(x1+2,x2-2,y2-1,refresh.running and "WAITING FOR COMPUTERS..." or "RESULTS WILL CLOSE AUTOMATICALLY",
        refresh.running and theme.pending or colors.lightGray,theme.popupBackground)
end

local function drawScreen()
    rebuildCards()
    local width,height=monitor.getSize(); fill(1,1,width,height,theme.background); drawHeader()
    local first=firstIndexOnPage(); local last=lastIndexOnPage(); local slotIndex=1
    for globalIndex=first,last do
        local slot=layoutSlots[slotIndex]; local itemID=cardOrder[globalIndex]
        if slot and itemID then
            local card=cards[itemID]
            card.x1=slot.x1; card.y1=slot.y1; card.x2=slot.x2; card.y2=slot.y2; card.borderPoints=slot.borderPoints
            drawCard(card)
        end
...
local function updateRefreshState()
    ...
    if refresh.running then
        if (tableCount(refresh.expected)>0 and tableCount(refresh.responses)>=tableCount(refresh.expected))
            ...
            refresh.running=false; refresh.finishedAt=time; pageChangedAt=time; screenDirty=true
        end
    elseif refresh.finishedAt>0 and time-refresh.finishedAt>=refreshResultSeconds*1000 then
        refresh.visible=false; pageChangedAt=time; screenDirty=true
    end
end

local function rebootComputer()
    drawRebootButton(theme.rebootPressed); sleep(0.15); os.reboot()
end

local function renderLoop()
    while true do
        updateRefreshState(); updatePageRotation()
        if screenDirty then screenDirty=false; drawScreen()
        elseif pageCount()>1 and not refresh.visible then drawHeader() end
...
local function animationLoop()
    while true do
        chaseFrame=chaseFrame+1; flashElapsed=flashElapsed+animationSpeed; local flashChanged=false
        if flashElapsed>=flashInterval then flashElapsed=flashElapsed-flashInterval; flashState=not flashState; flashChanged=true end
        if not refresh.visible then
            local first=firstIndexOnPage(); local last=lastIndexOnPage(); local slotIndex=1
            for globalIndex=first,last do
                local itemID=cardOrder[globalIndex]; local card=cards[itemID]; local slot=layoutSlots[slotIndex]
                if card and slot then
                    card.x1=slot.x1; card.y1=slot.y1; card.x2=slot.x2; card.y2=slot.y2; card.borderPoints=slot.borderPoints
                    if card.machineRunning then drawCardBorder(card)
                    elseif flashChanged then local p=calculatePercentage(card); if hasError(card) or (p and p<25) then drawCardBorder(card) end end
                end
                slotIndex=slotIndex+1
            end
        end
...
local function eventLoop()
    ...
    while true do
        ...
        elseif event=="monitor_touch" and a==monitorName then
            local x,y=b,c
            if not refresh.visible then
                if isInside(x,y,rebootButton.x1,rebootButton.y1,rebootButton.x2,rebootButton.y2) then rebootComputer()
                elseif isInside(x,y,refreshButton.x1,refreshButton.y1,refreshButton.x2,refreshButton.y2) then forceTargetRefresh() end
            end
        ...
        elseif event=="timer" and a==staleTimer then
            screenDirty=true; staleTimer=os.startTimer(2)
        elseif event=="monitor_resize" and a==monitorName then
            monitor.setTextScale(0.5); calculateLayout(); screenDirty=true
        end
    end
end
