...
local failureDisplayIndex=1
local failureDisplayChanged=0
...
local function loadDiskCache()
    for _,file in ipairs(managedFiles) do
        ...
        if data and data~="" then
            cache[file]=data
            cacheStatus[file]={source="DISK CACHE",updated=0,bytes=#data,lastError=nil}
        end
    end
end

local function classifyHttpError(code,data)
    if code==401 then return "401 TOKEN REJECTED" end
    if code==403 and githubRateRemaining==0 then return "403 RATE LIMIT EXHAUSTED" end
    if code==403 then return "403 FORBIDDEN" end
    if code==404 then return "404 FILE NOT FOUND" end
    if code==422 then return "422 GITHUB REQUEST REJECTED" end
    if code>=500 then return tostring(code).." GITHUB SERVER ERROR" end
...
local function downloadFromGitHub(file)
    ...
    if not response then
        local reason="HTTP CONNECTION FAILED: "..tostring(err)
    ...
    if code==401 then githubAuthStatus="TOKEN REJECTED" end

    if code<200 or code>=300 then
        local reason=classifyHttpError(code,data)
        cacheStatus[file]=cacheStatus[file] or {}
        cacheStatus[file].lastError=reason
        cacheStatus[file].lastAttempt=now()
        return false,reason
    ...
    if tostring(file):sub(-4)==".lua" then
        ...
        if not loader then
            local reason="LUA SYNTAX: "..tostring(syntaxError)
            cacheStatus[file]=cacheStatus[file] or {}
            cacheStatus[file].lastError=reason
            cacheStatus[file].lastAttempt=now()
            return false,reason
...
local function refreshFileIfNeeded(file,forceFresh)
    ...
    local age=status and status.updated and status.updated>0 and (now()-status.updated) or math.huge
...
local function printRefreshReport(successes,failures)
    ...
    for _,failure in ipairs(failures) do
        ...
        print("  "..failure.error)
        if cache[failure.file] then
            term.setTextColor(colors.yellow)
            print("  FALLBACK: cached copy available")
        else
            term.setTextColor(colors.red)
            print("  FALLBACK: NO CACHED COPY")
        end
        term.setTextColor(colors.white)
    end
end

local function refreshAll(showTerminalReport)
...
local function currentFailure()
    ...
    if now()-failureDisplayChanged>=4000 then
        failureDisplayIndex=failureDisplayIndex+1
        if failureDisplayIndex>#lastFailures then failureDisplayIndex=1 end
        failureDisplayChanged=now()
    end
    return lastFailures[failureDisplayIndex]
end
