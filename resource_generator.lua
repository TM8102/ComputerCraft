...
-- Controls machines only. No storage scanning.
-- =========================================================
...
local function validateConfig(config)
    ...
    for itemID, settings in pairs(config) do
        ...
        if type(settings) ~= "table" then
            return false, "Invalid settings for " .. tostring(itemID)
        end
        ...
        if settings.machine then
            ...
            if not machine.computerID then
                return false, "Invalid computerID for " .. itemID
            end
            if type(machine.machineKey) ~= "string" or machine.machineKey == "" then
                return false, "Invalid machineKey for " .. itemID
            end
...
local function rebuildMachines()
    ...
    for itemID, settings in pairs(itemConfig) do
        ...
        if machine and tonumber(machine.computerID) == computerID then
            ...
            resourceByItem[itemID] = {
                itemID = itemID,
                displayName = getDisplayName(itemID),
                side = side,
                machineKey = machine.machineKey,
                target = tonumber(settings.target) or 0,
                amount = nil,
                percent = nil
            }
        end
    ...
    for side, machine in pairs(machineBySide) do
        redstone.setOutput(side, machine.state)
    end
end
    ...
    for _, itemID in ipairs(machine.itemIDs) do
        ...
        if resource then
            rednet.broadcast({
                ...
                machineKey = resource.machineKey,
                side = machine.side,
...
local function sendAllMachineStatus()
    for _, machine in pairs(machineBySide) do
...
local function applyCommandData(message, machine)
    if type(message.itemIDs) == "table" then
        for _, itemID in ipairs(message.itemIDs) do
            local resource = resourceByItem[itemID]
            if resource then
                if type(message.amounts) == "table" then
                    resource.amount = tonumber(message.amounts[itemID])
                end
                if type(message.percentages) == "table" then
                    resource.percent = tonumber(message.percentages[itemID])
                end
                if type(message.targets) == "table" then
                    resource.target = tonumber(message.targets[itemID]) or resource.target
                end
            end
        end
    elseif type(message.itemID) == "string" then
        ...
        if resource then
            resource.amount = tonumber(message.amount)
            resource.percent = tonumber(message.percent)
            resource.target = tonumber(message.target) or resource.target
        end
    end
end
...
local function drawScreen()
    ...
    for _, side in ipairs(sides) do
        ...
        if machine then
            found = true
            print(" " .. string.upper(side) .. " (" .. #machine.itemIDs .. " resource" .. (#machine.itemIDs == 1 and "" or "s") .. ")")
            ...
            for _, itemID in ipairs(machine.itemIDs) do
                local resource = resourceByItem[itemID]
                if resource then
                    local detail = "   " .. resource.displayName
                    if resource.percent then
                        detail = detail .. " " .. string.format("%.1f%%", resource.percent)
                    end
                    print(detail)
                end
            end
